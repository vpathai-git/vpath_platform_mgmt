...
def override_generated_np(app):
    ...
    (d / "networkpolicy.yaml").write_text(
        ...
        "kind: NetworkPolicy\n"
        "metadata:\n"
        "  name: vpath-storage-demo-allow-egress\n"
...
def handrolled_storage_hook(app):
    """C-given-storage-hooks: define a local storage hook instead of consuming
    the SDK (useStorageRoots / useStorageListing)."""
    p = app / "src" / "components" / "rogue-storage.tsx"
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(
        "const useStorageListing = () => {\n"
        "  return [];\n"
        "};\n"
        "export default useStorageListing;\n",
        encoding="utf-8",
    )


def bad_folder_grammar(app):
    """C-need-app-storage: give an appStorage folder an out-of-grammar `shared`
    value (only private/read/readwrite are valid)."""
    p = app / "vpath-app.yaml"
    if not p.exists():
        raise FileNotFoundError(f"mutation target missing: {p}")
    text = p.read_text(encoding="utf-8")
    if "        shared: private" not in text:
        raise AssertionError("appStorage folder 'shared: private' not found")
    p.write_text(
        text.replace("        shared: private", "        shared: public"),
        encoding="utf-8",
    )


def bad_shared_folder(app):
    """C-need-shared-folders: declare a sharedFolders[] entry with an
    out-of-grammar access (only read/readwrite are valid)."""
    p = app / "vpath-app.yaml"
    if not p.exists():
        raise FileNotFoundError(f"mutation target missing: {p}")
    text = p.read_text(encoding="utf-8")
    ...
    if anchor not in text:
        raise AssertionError("capabilities block not found to anchor sharedFolders")
    inject = (
        "  sharedFolders:\n"
        "    - name: Joint\n"
        "      path: /var/lib/vpath-storage-demo/joint\n"
        "      access: private\n"
    ) + anchor
    p.write_text(text.replace(anchor, inject, 1), encoding="utf-8")
...
MUTATIONS = {
    ...
    "handrolled-storage-hook": handrolled_storage_hook,
    "bad-folder-grammar": bad_folder_grammar,
    "bad-shared-folder": bad_shared_folder,
}
